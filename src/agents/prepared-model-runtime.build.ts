...
function createFullModelCatalogAccess(params: {
  ...
  return {
    ...
    loadAuth: ({ providerIds, profileIds }) => {
      ...
      const promise = (async () => {
        // oxlint-disable-next-line no-underscore-dangle -- This scope only owns disposal.
        await using _lease = {
...
async function buildSnapshotBatch(
  ...
  try {
    ...
    if (failures.length) {
      // oxlint-disable-next-line preserve-caught-error -- AggregateError retains the original error alongside cleanup failures.
