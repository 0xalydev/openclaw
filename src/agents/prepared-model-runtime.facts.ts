...
export async function prepareWorkspaceBuildGroup(
  ...
  // oxlint-disable-next-line no-underscore-dangle -- The scope retains metadata, without reading its token.
  using _preparingMetadata = {
  ...
  try {
    ...
    if (failures.length) {
      // oxlint-disable-next-line preserve-caught-error -- The original error is an aggregate member.
