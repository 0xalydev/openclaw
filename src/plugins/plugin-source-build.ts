  ...
  try {
    ...
    const compile = (input: string, destination: string, format: string | null | undefined) => {
      ...
      if (!diagnostics.some((d) => d.category === ts.DiagnosticCategory.Error)) {
        if (sources.get(destination)?.mode === "sync" && programSource) {
          // Preserve Jiti's maintained sync source syntax and CJS bindings; Node still evaluates it.
          ...
          if (needsHelper(programSource)) {
            helper = path.join(path.dirname(destination), `.import-meta-${randomUUID()}.mjs`);
            fs.writeFileSync(
              helper,
...
export const bindRequire = (nativeRequire) => {
  ...
  const require = (specifier) => nativeRequire(select(specifier).target);
...
export const importModule = async (specifier) => {
  ...
  if (new URL(resolved).pathname.endsWith(".json")) return namespace.default;
...
`,
          ...
          try {
            for (const emittedFile of emitted.transformed) {
              fs.writeFileSync(destination, ts.createPrinter().printFile(emittedFile), {
                mode: 0o600,
              });
            }
          } finally {
