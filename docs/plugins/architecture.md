...
A managed runtime instance owns its source artifact, module results, registered
callables, and runtime-store slots. Package plugins capture their package inputs
when the instance is created. Standalone files and compiled setup artifacts
...
Dynamic TypeScript imports use asynchronous source execution, while source loaded
from native JavaScript follows Node's module format. The first evaluation fixes
that mode for the instance; resolving a module alone does not evaluate it. Use ESM
exports for asynchronous TypeScript modules. Jiti-specific asynchronous CommonJS
wrapper behavior and optional `import.meta.resolve` arguments are not emulated;
the standard one-argument resolver uses the source's directory and package scope.
