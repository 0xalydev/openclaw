...
class QaCrablineTransport extends QaStateBackedTransportAdapter {
  ...
  readonly createRuntimeBootstrap?: (repoRoot: string) => {
    modulePath: string;
    payload: string;
  };
  ...
  constructor(params: {
    ...
    if (params.adapter.manifest.provider === "discord") {
      const { endpoints } = params.adapter.manifest;
      this.createRuntimeBootstrap = (repoRoot) => ({
        modulePath: path.join(repoRoot, "scripts", "qa", "discord-endpoint-bootstrap.mjs"),
        payload: JSON.stringify({
          restApiBaseUrl: `${endpoints.apiRoot}/v10`,
          gatewayBotUrl: endpoints.gatewayBotUrl,
          gatewayOrigin: new URL(endpoints.gatewayUrl).origin,
        }),
      });
    }
