...
export class MemoryManagerRegistry<T extends ClosableMemoryManager> {
  ...
  prepareReload(
    ...
    return {
      ...
      resume: () => {
        if (this.reload !== reload || generation !== reload.generation) {
          return;
        }
...
/** Managed calls share their exact instance slot; standalone CLI calls share its default slot. */
export function getMemoryIndexManagerRegistry(): MemoryManagerRegistry<MemoryIndexManager> {
  ...
  if (!registry) {
    registry = new MemoryManagerRegistry();
